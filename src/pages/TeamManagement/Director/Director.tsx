...
import CreateDirector from '@/pages/components/form/form-create-director';
...
const Director: React.FC = () => {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ['getListDirectorManagement'],
    queryFn: () => getListManagement('2'),
  });
  ...
  const useDirector = () => {
    const create = useMutation(createManagement, {
      onSuccess: () => {
        queryClient.invalidateQueries(['getListDirectorManagement']);
  ...
  return (
    <div className="aaa">
      <HeadTitle title="Quản lý đội nhóm cấp giám đốc" />
      ...
      >
        <CreateDirector initForm={customerSelect} useSale={useDirector} />
      </Drawer>
    </div>
  );
};
