...
const CreateDirector: React.FC<ICreateUser> = ({ initForm, useSale }) => {
  ...
  useEffect(() => {
    if (saleData?.data?.data) {
      const managerOptions = saleData?.data?.data?.rows?.filter((item: any) => item?.SaleLevel?.level === 1);
